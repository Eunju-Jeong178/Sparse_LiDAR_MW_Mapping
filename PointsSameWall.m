...
%     c = walls.refittedLineModel(3);
    ...
    if walls.alignment == 'x'
        y1 = walls.min_xy_M(2);
%         x1 = (y1 - walls.refittedLineModel(3))/walls.refittedLineModel(1);
        x1 = walls.offset;
        y2 = walls.max_xy_M(2);
%         x2 = (y2 - walls.refittedLineModel(3))/walls.refittedLineModel(1);
        x2 = walls.offset;
        lineEndPoint1 = [x1; y1];
        lineEndPoint2 = [x2; y2];
...
%         y1 = walls.refittedLineModel(1)*x1 + walls.refittedLineModel(3);
        y1 = walls.offset;
        x2 = walls.max_xy_M(1);
%         y2 = walls.refittedLineModel(1)*x2 + walls.refittedLineModel(3);
        y2 = walls.offset;
        lineEndPoint1 = [x1; y1];
        lineEndPoint2 = [x2; y2];
    end
...
%     end
    if walls.alignment == 'x'
        for k = 1:numPoint
            distanceEachPoint(k) = abs(pointCloud(1,k) - walls.offset); % |각 point의 x value  - offset|
        end
    elseif walls.alignment == 'y'
        for k = 1:numPoint
            distanceEachPoint(k) = abs(pointCloud(2,k) - walls.offset); % |각 point의 y value  - offset|
        end
    end


    pointsIdxInThres = find(distanceEachPoint <= TH_DISTANCE_BETWEEN_REFITTED_LINE); % 점과 직선 사이의 거리가 TH 이하라면 그 점들을 포함
